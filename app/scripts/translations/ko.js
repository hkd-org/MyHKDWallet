...
GAS_PRICE_Desc              : '가스 가격은 가스 단위당 지불할 금액입니다. `TX 수수료 = 가스 가격 * 가스 한도`이며 TX를 포함한 블록을 채굴한 채굴자에게 수수료가 지급됩니다. 높은 가스 가격 = 트랜잭션은 빨라지나 가격은 비쌈. 기본값은 `12 GWEI`.',
...
HELP_Remind_Desc_1          : '**이더리움, MyEtherWallet.com & MyHkdWallet CX, 그리고 우리가 사용하는 JavaScript 라이브러리는 활발하게 개발되고 있습니다. ** 우리가 철저하게 테스트를 진행하여 수만 개의 지갑이 전 세계 사람들에 의해 성공적으로 만들어졌지만 예상치 못한 일이 발생하여 ETH를 잃을 가능성이 항상 존재합니다. 항상 조심해주세요. 만약 무슨 일이 발생한다면, 유감스럽지만 **우리는 잃어버린 이더리움에 대한 책임을 지지 않습니다.** ',
...
HELP_8CX_Desc_2             : '즉, **매우 중요** MyHkdWallet CX 로 새로 만든 지갑 정보는 모두 백업해 두어야 합니다. 만약 컴퓨터에 문제가 생기거나 Chrome 확장 프로그램에 문제가 생겨도 백업해둔 파일로 지갑에 접근할 수 있습니다. #2a에서 지갑을 어떻게 백업 하는 지 확인할 수 있습니다. ',
HELP_8CX_Desc_3             : '어떤 이유로 MyHkdWallet CX 가 Chrome 스토어에서 사라져도, Github 에서 소스를 다운 받아 수동으로 사용할 수 있습니다. 위의 #5를 확인해주세요. ',
...
HELP_SecCX_Title            : '보안 - MyHkdWallet CX ',
